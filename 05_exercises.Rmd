...
ggmap(mallorca_map)+
  ...
  labs(title= "Mallorca biking path",
       subtitle = "Time: {closest_time}")+
  theme_map()+
  theme(legend.background = element_blank(),
        legend.position = "bottom")

# confused on what animation to add 
```
...
ggmap(panama_map)+
  geom_point(data = panama_races,
             ...
             size=.5)+
  scale_color_manual(values = c("Bike"="red",
                                "Run"= "green",
                                "Swim"= "blue"))+
...
covid_per_state <- covid19 %>%
  ...
  filter(date == "2021-06-20") # filter by every friday instead? how??
...
covid_anim2 <- covid_with_2018_pop_est %>% 
  ggplot(aes(x=cases, y = covid_per_10000, color=state, group= date))+
  geom_path()+
  geom_point(size = 2.5)+
  geom_text(aes(label = state),
            check_overlap = TRUE)+
  labs(x = 'Cases',
       y= "",
       title = "Covid Cases per 10.000 people",
       subtitle = "Date: {frame_along}")+
  theme(legend.position = "none")+
  transition_reveal(date)

animate(covid_anim2, duration= 30, nframes=200)
```
